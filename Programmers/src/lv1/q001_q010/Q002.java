...
public class Q002 {
     ...
     * 제한수치를 초과한 기사가 사용할 무기의 공격력이 2라면, 15번으로 지정된 기사단원은 
     * 무기점에서 공격력이 2인 무기를 구매합니다. 무기를 만들 때, 무기의 공격력 1당 1kg의 철이 필요합니다. 
    ...
    public int solution(int number, int limit, int power) {
        ...
        while(currentNum <= number){
            int count = 0;
            for(int i = 1; i <= Math.sqrt(currentNum); i++){
                if(currentNum%i == 0){
                    count += (currentNum/i == i)? 1 : 2;
                }
            }
